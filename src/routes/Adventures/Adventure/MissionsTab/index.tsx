...
const MissionsTab: React.FC = () => {
  ...
  const [selectedMission, setSelectedMission] = useState<IMission | undefined>(undefined);

  useEffect(() => {    
    if (shownStage && !missionsByStage[shownStage.id]) {      
      getMissionsByStage(shownStage.id)
      .then(({ data }) => setMissionsByStage((missionsByStage) => ({ ...missionsByStage, [shownStage.id]: data })))
      .catch((e) => console.log(e));
    }
  }, [shownStage]);

  const handleStageChange = (stage: IStage) => setShownStage(stage);
  const handleSelectMission = (mission: IMission | undefined) => setSelectedMission(mission);
    ...
    <MissionsListContainer>
      {adventure?.stages && shownStage ? (
        <div>
              ...
              {adventure?.stages?.map((stage, i) => (
                <Fragment key={`${adventure.id}-${stage.id}`}>
                  <StagesStepper className="d-flex flex-column align-items-center" role="button" onClick={() => handleStageChange(stage)}>
                    ...
                    <Typography variant="caption" className="mt-1">{`Etapa ${stage.index}`}</Typography>
                  </StagesStepper>
          ...
          <MissionDialog  PaperProps={{ sx: { borderRadius: 2, overflow: 'visible' }}} open={!!selectedMission} onClose={() => handleSelectMission(undefined)}>
            {selectedMission ? (
              <MissionCard title={selectedMission.title} description={selectedMission.description} qr={selectedMission.qr} points={20} icon={selectedMission.skill?.icon || ''} color={selectedMission.skill?.color || '#000'} />
            ) : null}
          </MissionDialog>
        </div>
      ) : null}      

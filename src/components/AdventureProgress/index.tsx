...
const AdventureProgress: FC<{ adventure: IAdventure, progressPercentage: number, averageCompletedMission: number }> = ({ adventure, progressPercentage = 0, averageCompletedMission = 0 }) => {
  ...
  return (
    <AdventureProgressContainer className="d-flex align-items-center justify-content-between p-5" sx={{ backgroundImage: `url(${latestStage?.icon})` }}>
      <Box>
        <Box
          display={"flex"}
        ...
        <Typography variant="h5" fontWeight="bold">{`${classDetails.current_adventure?.title}`}</Typography>
        <Typography variant="body1" fontWeight="bold" className="mb-4">{`Etapa ${latestStage?._index}: ${latestStage?.title}`}</Typography>

...
const OnboardingContextProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <OnboardingContext.Provider value={{ setNewAvailableTours }}>
      { children }
      <TourFab color="primary" onClick={handleClick}>
        <QuestionMarkIcon />
      </TourFab>
      <Menu
        anchorEl={anchorEl}
      ...
      >
        ...
        {availableTours?.map((tour, i) => <MenuItem key={`tour-item-${i}`} onClick={() => handleItemClick(tour.steps)}>{tour.name}</MenuItem>)}
      </Menu>
